...
class enrol_database_plugin extends enrol_plugin {
    ...
    public function sync_user_enrolments($user) {
        ...
        /*<change>*/
        $grpnamesfield = trim($this->get_config('remotegrpnamesfield'));
        $grpnamesfield = (empty($grpnamesfield) || is_null($grpnamesfield) || !isset($grpnamesfield)) ? "grpnames" : $grpnamesfield;
        $grpseparator = trim($this->get_config('remotegrpseparator'));
        $grpseparator = (empty($grpseparator) || is_null($grpseparator) || !isset($grpseparator)) ? "," : $grpseparator;
        /*</change>*/
        ...
        /*<change>*/
        $grpnamesfield_l = strtolower($grpnamesfield);
        /*</change>*/
                ...
                while ($fields = $rs->FetchRow()) {
                    ...
                    /*<change>*/
                        ...
                        if (groups_get_all_groups($course->id, $user->id)) {
                            $currentgroup[$ue->userid] = true;
                        }

                        //debugging('Iteration-'.$grpnamesfield_l.'-'.$fields[$grpnamesfield_l]);var_dump($fields);
                        $grpnames_str = $fields[$grpnamesfield_l];
                        if (!empty($grpnames_str) || $currentgroup == true) {
                            // Requires GROUPS management?
                            // Getting group names as array
                            $grpnames = explode($grpseparator, $grpnames_str);
                            
                            /*<change>*/
                            // User already is in other group?

                            foreach (groups_get_all_groups($course->id, $user->id) as $usergroups) {
                                // Sistemas - Para no eliminar alumnos en multiples grupos
                                    //groups_remove_member($usergroups->id,$user->id); //Remove user from group
                            }
                            /*</change>*/
                            
                            // Manage each group coming from  remote user enrolment record
                            if (!empty($grpnames_str)) {
                                foreach ($grpnames as $gn) {
                                    //$gn = utf8_encode($gn);

                                    if (!$gid=groups_get_group_by_name($course->id, $gn)) { // Group already exists?
                                        // Does not exist -> create
                                        $grp = new stdClass();
                                        $grp->name = $gn;
                                        $grp->courseid = $course->id;
                                        $gid = groups_create_group($grp);
                                    }
                                    
                                    if (!groups_is_member($gid, $user->id)) {   // User already member?
    ...
    public function sync_enrolments(progress_trace $trace, $onecourse = null) {
        ...
        //
        $groupfield = trim($this->get_config('remotegrpnamesfield'));
        ...
        $sqlfields = array($userfield, $groupfield);
        if ($rolefield) {
            $sqlfields[] = $rolefield;
        }
        if ($otheruserfield) {
	    ...
	    $sql = "SELECT u.$localuserfield AS mapping, u.id AS userid, ue.status, ra.roleid
            ...
            foreach ($rs as $ue) {
                ...
                if (isset($ue->status)) {
                    ...
                    if (groups_get_all_groups($course->id, $ue->userid)) {
                        $currentgroup[$ue->userid] = true;
                    }
                }
            }
		...
		if ($rs = $extdb->Execute($sql)) {
                if (!$rs->EOF) {
                    ...
                    while ($fields = $rs->FetchRow()) {
                        ...
                        // Sistemas - Multiples grupos en un mismo curso (caso optativas RRII)
                        (array_key_exists($userid, $requestedgroup)) ? $requestedgroup[$userid] .= ",".$fields[$groupfield] : $requestedgroup[$userid] = $fields[$groupfield];
                        // $requestedgroup[$userid] .= $fields[$groupfield];
            ...
            foreach ($requestedenrols as $userid => $userroles) {
                foreach ($userroles as $roleid) {
                    ...
                    $grpnames_str = $requestedgroup[$userid];

                    $grpnames = explode($grpseparator, $grpnames_str);


                    if (!empty($requestedgroup[$userid]) || $currentgroup == true) {    // Requires GROUPS management?
                        // User already is in other group?

                        foreach (groups_get_all_groups($course->id, $userid) as $usergroups) {
                            groups_remove_member($usergroups->id, $userid); //Remove user from group
                        }
                        ...
                        if (!empty($grpnames_str)) {
                            ...
                            foreach ($grpnames as $gn) {
                                ...
                                //$trace->output("Nombre de grupo ".$gn, 1);
                                ...
                                if (!groups_is_member($gid, $userid)) {  // User already member?
    ...
    /**
     * This function adds a user to a group, first creating the group if it didn't exist
     *
     * @param object $course
     * @param string $groupn Name of group.
     * @param int $userid Id of the user that should be added to the group.
     * @return boolean
     */
    protected function enrol_to_group($courseid=null, $groupname='', $userid=null)
    {
        if (empty($courseid) || empty($groupname) || empty($userid)) {
            error_log('[ENROL_GRP] Processing group '.$groupna.' for course '.$courseid.' and user '.$userid);
            return false;
        }

        if ($groupname = $groupname) { //FV: 1) Get the group from the database
            if (!$existinggroup = groups_get_group_by_name($courseid, $groupname)) { //The group doesn't exist, so we should try to create it.
                $newgroup = new stdClass();
                $newgroup->name = $groupname;
                $newgroup->courseid = $courseid;
                $newgroup->timecreated = time();
                //Create the group
                if (!($newgroup->id = groups_create_group($newgroup))) {
                    //Group not added for any reason
                    error_log('[ENROL_GRP_FLACSO] --'.get_string('groupnotaddederror', 'error', $newgroup->name));
                } else {
                    error_log('[ENROL_GRP_FLACSO] --'.get_string('groupaddedsuccesfully', 'group', $newgroup->name));
                    $existinggroup = $newgroup->id;
                }
            }
            error_log('[ENROL_GRP_FLACSO] newgroup '.serialize($existinggroup));
            //FV: 3) Add the user to the group
            if (!groups_is_member($existinggroup, $userid)) {
                if (!groups_add_member($existinggroup, $userid)) {
                    //Group not added for any reason
                    error_log('[ENROL_GRP_FLACSO] '.get_string('usernotaddederror', 'error', $userid));
                    return false;
                }
            } else {
                error_log("[ENROL_GRP_FLACSO] $userid Ya es miembro del grupo $existinggroup");
                return false;
            }
            return true;
        } else {
            //error_log('[ENROL_GDB] weird failure');
            return false;
        }
    }
}
