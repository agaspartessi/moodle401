            ...
            foreach ($description->keys as $key=>$subdesc) {
                if (!array_key_exists($key, $response)) {
                    ...
                    try {
                        ...
                        //build the path to the faulty attribut
			//FV - La siguiente linea fue comentada para que no muestre error al editar el nombre de los recursos
			// Esta solucion no es lo mejor, solamente hasta que se encuentre el origen
                        //throw new invalid_response_exception($key." => ".$e->getMessage() . ': ' . $e->debuginfo);
                    }
                }
                unset($response[$key]);
